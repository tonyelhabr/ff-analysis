...
def get_scheduled_fixtures(solver, fixtures, pools):
    pool_membership = {
        home: homepool
        for (homepool, pool) in enumerate(pools) for home in pool
    }
    ...
    fixed_matches = [
        ...
        } for (day, fd) in enumerate(fixtures) for (home, fh) in enumerate(fd)
        for (away, fixture) in enumerate(fh) if solver.Value(fixture)
    ]
...
def screen_dump_poolchecks(scheduled_games, pools):
    ...
    pool_vs_pool = [[0 for j in pools] for i in pools]
    ...
    [
        print(
            'team %i (home or away) versus pool %i = %i' %
            (t + 1, j, team_vs_pool[t][j])
        ) for i in range(len(pools)) for t in pools[i]
    ...
    for i in range(len(pools)):
        for j in range(len(pools)):
            print(
                'pool %i at home vs pool %i away, count = %i' %
                (i, j, pool_vs_pool[i][j])
            )
...
def collect_pool_play_fixtures(teams, pools, matchdays, fixtures):
    pool_play = [
        [
            fixture_slice(fixtures, matchdays, [t], pool) +
            fixture_slice(fixtures, matchdays, pool, [t]) for pool in pools
        ] for t in teams
    ]
    return pool_play
...
def collect_pool_balance_fixtures(pools, matchdays, fixtures):
    pool_balance = [
        [fixture_slice(fixtures, matchdays, pooli, poolj) for poolj in pools]
        for pooli in pools
    ]
    return pool_balance
...
def fixture_slice(fixture, days, homes, aways):
    return [
        fixture[day][home][a] for day in days for home in homes for a in aways
    ]
...
def season_expected_games(
    games_per_rr, matchups_exact, matchups, total_games, unique_games
):
    if matchups_exact:
...
def add_pool_play_constraints(pools, pool_play, model, minimum_games_function):
    ...
    def constrain_games(triple):
        ...
        minimum_expected_games = minimum_games_function(
            games_per_rr=round_robin_games
        )
    ...
    result = list(
        map(
            constrain_games, [
                (t, ppi, pooli) for t in range(len(pool_play))
...
def add_pool_balance_constraints(
    pools, pool_balance, model, minimum_games_function
):
    ...
    result = list(
        map(
            constrain_games, [
                (ppi, pooli, ppj, poolj) for (ppj, poolj) in enumerate(pools)
                for (ppi, pooli) in enumerate(pools)
            ]
        )
    )
...
def add_one_matchup_per_round_robin(
    teams, fixtures, model, matchups, matchups_exact, unique_games,
    matches_per_day, num_matchdays
):
    ...
    [
        ...
        for t in teams for o in teams if t != o for mdays in rr_days
    ]
    ...
    if matchups_exact:
        [
            model.Add(
                sum(fn(last_days, [t], [o]) + fn(last_days, [o], [t])) == 1
            ) for t in teams for o in teams if t != o
        ]
    else:
...
def add_max_home_stand_constraint(
    teams, at_home, model, num_matchdays, max_home_stand
):
...
def create_breaks(model, teams, num_matchdays):
    ...
    breaks = [
        ...
        enumerate([(t, d) for t in teams for d in range(num_matchdays - 1)])
    ]
    return breaks


def breaks_constraint(
    breaks, teams, at_home, num_matchdays, model, listall=False
):
    # side effects through list comprehension, yet again
    [
        [
            model.AddBoolOr(
                [at_home[d][t], at_home[d + 1][t], breaks[breakidx]]
            ),
            model.AddBoolOr(
                [
                    at_home[d][t].Not(), at_home[d + 1][t].Not(),
                    breaks[breakidx]
                ]
            ),
            model.AddBoolOr(
                [
                    at_home[d][t].Not(), at_home[d + 1][t],
                    breaks[breakidx].Not()
                ]
            ),
            model.AddBoolOr(
                [
                    at_home[d][t], at_home[d + 1][t].Not(),
                    breaks[breakidx].Not()
                ]
            )
        ] for (breakidx, (t, d)) in
...
def initialize_pools(num_pools, num_teams):
    ...
    pools = [
        (list(range(int(g * pool_size), int((g + 1) * pool_size))))
        for g in range(num_pools - 1)
    ]
...
def opponent_fixtures(model, num_teams, day, home_team):
    ...
    return [
        model.NewBoolVar(name_prefix + 'away %i' % away)
...
def home_fixtures(model, num_teams, day):
    opp_fix = partial(
        opponent_fixtures, model=model, num_teams=num_teams, day=day
    )
...
def daily_fixtures(model, num_teams, num_days):
    return daily_thing(
        home_fixtures, model=model, num_teams=num_teams, num_days=num_days
    )
...
def create_at_home_array(model, num_teams, day):
    ...
    return [
        model.NewBoolVar(name_prefix + 'home %i' % home)
...
def model_matches(
    num_teams, num_matchdays, num_matches_per_day, num_pools, max_home_stand,
    listall
):
    ...
    fixtures = daily_fixtures(
        model, num_teams, num_matchdays
    )  # all possible games
    at_home = daily_at_home(
        model, num_teams, num_matchdays
    )  # all possible games
    ...
    [
        ...
        ) for d in matchdays for i in teams for j in teams if i != j
    ]
    ...
    minimum_games_function = partial(
    ...
    # assert 0
    add_one_matchup_per_round_robin(
        teams, fixtures, model, matchups, matchups_exact, unique_games,
        num_matches_per_day, num_matchdays
    )

    add_max_home_stand_constraint(
        teams, at_home, model, num_matchdays, max_home_stand
    )
...
def solve_model(model, time_limit=None, num_cpus=None, debug=None):
    # run the solver
...
def report_results(
    ...
    if status != cp_model.OPTIMAL and solver.WallTime() >= time_limit:
        print(
            'Please note that solver reached maximum time allowed %i.' %
            time_limit
        )
        print(
...
def main():
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        'How many separate pools should the teams be separated into.  Default is 1'
    )
    ...
    parser.add_argument(
        ...
        dest='listall',
        help=
        "Enumerate all possible cases schedules, instead of finding just one.  This will create an absurd number of schedules for any reasonably-sized problem."
    )
    ...
    # set up the model
    (pools, fixtures, breaks, model) = model_matches(
        args.num_teams, args.num_matchdays, num_matches_per_day, args.num_pools,
        args.max_home_stand, args.listall
    )
    ...
    if not args.listall:
        ...
        report_results(
            solver, status, fixtures, pools, args.num_teams, args.num_matchdays,
            args.time_limit, args.csv
        )
    else:
        (solver, status) = solution_search_model(
            model, fixtures, pools, args.time_limit, cpu, args.debug, args.csv
        )
